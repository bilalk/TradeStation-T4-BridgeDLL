...
public sealed class RealT4Connector : IT4Connector
{
    ...
    public string Connect(BridgeConfig cfg)
    {
        ...
        try
        {
            ...
            // Select simulator vs. production based on host string
    ...
    public string PlaceOrder(string symbol, string side, int quantity, decimal price, string orderType)
    {
        ...
        try
        {
            // Placeholder: real order submission via T4.API would go here (stub path).
    ...
    private void OnLoginResponse(T4.LoginResponseEventArgs e)
    {
        if (e.Result == T4.LoginResult.Success)
        {
            _connected = true;
        }
        else
