...
// ---------------------------------------------------------------------------
// Module directory helper
// ---------------------------------------------------------------------------
// Returns the directory (ending with backslash) of the module that contains
// this translation unit.  Uses this free function's own address as a valid
// non-member-function pointer for GetModuleHandleExA, avoiding C2276/C2660
// errors that arise when passing a pointer-to-member-function.
...
private:
    ...
    bool SpawnWorker() {
        ...
        if (workerPath.empty()) {
            ...
            workerPath = GetModuleDirectory() + "dotnet\\BridgeDotNetWorker\\BridgeDotNetWorker.exe";
        }
