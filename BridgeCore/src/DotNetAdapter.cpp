...
// ---------------------------------------------------------------------------
// Module-anchor helper
// A free (non-member) function whose address lies inside this DLL/module.
// Used with GET_MODULE_HANDLE_EX_FLAG_FROM_ADDRESS so that GetModuleHandleExA
// can locate the HMODULE for this binary without relying on a member-function
// pointer (which causes C2276 on MSVC).
// ---------------------------------------------------------------------------
static void DotNetAdapterModuleAnchor() {}
...
private:
    ...
    bool SpawnWorker() {
        ...
        if (workerPath.empty()) {
            ...
            // Use DotNetAdapterModuleAnchor (a free function in this translation
            // unit) to obtain the HMODULE for this DLL via
            // GET_MODULE_HANDLE_EX_FLAG_FROM_ADDRESS.  A member-function pointer
            // cannot be cast to LPCSTR (MSVC C2276), so a free function is used.
            char dllPath[MAX_PATH] = {};
            HMODULE hMod = nullptr;
            GetModuleHandleExA(
                GET_MODULE_HANDLE_EX_FLAG_FROM_ADDRESS | GET_MODULE_HANDLE_EX_FLAG_UNCHANGED_REFCOUNT,
                reinterpret_cast<LPCSTR>(DotNetAdapterModuleAnchor), &hMod);
